...
public class FwSecurityService {
    ...
    public void checkHasPrivilege(Class<?> clazz, String op) {
        checkHasPrivilege(package2mod(clazz.getPackage()), objectname(clazz.getSimpleName()), objectname(op));
    }
    ...
    public List<FwPrivilegeVo> crudPrivileges() {
        return ClassUtil.scanPackage("com.wamogu.entity").stream()
                ...
                .map(x -> List.of(
                        ...
                        String.format(
                                ...
                                StrUtil.blankToDefault(
                                        x.getAnnotation(Table.class).comment(), x.getSimpleName()))))
