...
public class CodegenApp {
    ...
    public void main(String[] args) {
        ClassUtil.scanPackage("com.wamogu.entity").stream()
    ...
    private void generate(Class<?> enClass) {
        ...
        map.put(
                SUFFIX.REPOSITORY,
                ClassName.get("com.wamogu.dao.repository", enClass.getSimpleName() + SUFFIX.REPOSITORY));
    ...
    private ClassName createCastor(
            Class<?> enClass, List<String> projects, Map<String, ClassName> map, boolean override) {
        ...
        TypeSpec.Builder typeSpecBuilder = TypeSpec.interfaceBuilder(castorClassName)
                .addModifiers(Modifier.PUBLIC)
                .addSuperinterface(
                        ParameterizedTypeName.get(BASE_CLASS.classBaseCastor, entityClass, dtoClass, voClass))
    ...
    private ClassName createBizService(
            Class<?> enClass, List<String> projects, Map<String, ClassName> map, boolean override) {
        ClassName entityClass = ClassName.get(enClass);
        ...
        TypeSpec.Builder typeSpecBuilder = TypeSpec.classBuilder(bizServiceClassName)
                .superclass(ParameterizedTypeName.get(
                        BASE_CLASS.classBaseBizService, entityClass, dtoClass, voClass, pkClass))
        ...
        fields.entrySet().forEach(x -> {
            typeSpecBuilder.addField(FieldSpec.builder(x.getValue(), x.getKey(), Modifier.PRIVATE, Modifier.FINAL)
                    .build());
        });
    ...
    private ClassName createBizController(
            Class<?> enClass, List<String> projects, Map<String, ClassName> map, boolean override) {
        ...
        String comment = CharSequenceUtil.blankToDefault(
                enClass.getAnnotation(Table.class).comment(), enClass.getSimpleName());
        ...
        TypeSpec.Builder typeSpecBuilder = TypeSpec.classBuilder(controllerClassName)
                .superclass(ParameterizedTypeName.get(
                        BASE_CLASS.classBaseController, entityClass, dtoClass, voClass, queryClass, pkClass))
                .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
                .addAnnotation(Getter.class)
                ...
                .addAnnotation(AnnotationSpec.builder(RequestMapping.class)
                        .addMember("value", "$S", "/" + CharSequenceUtil.toUnderlineCase(enClass.getSimpleName()))
                        .build());
    ...
    private ClassName createDto(Class<?> enClass, List<String> projects, Map<String, ClassName> map, boolean override) {
        ...
        List<Class<? extends Object>> supportedClasses = List.of(
                Integer.class,
                int.class,
                Double.class,
                double.class,
    ...
    private ClassName writeJavaFile(
            String pack, TypeSpec.Builder typeSpecBuilder, List<String> projects, boolean override) throws IOException {
        ...
        for (String prj : projects) {
            String filepath = String.format(
                    "%s/%s/src/main/java/%s/%s.java",
                    baseDir, prj, javaFile.packageName.replaceAll("[.]", "/"), javaFile.typeSpec.name);
